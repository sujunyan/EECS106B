...
void PointCloudAnalyzer::setup_vis(){
	...
	//contact_membrane = (PointCloud::Ptr) new pcl::PointCloud<pcl::PointXYZRGB>;
    ...
    for (int i=0; i< detectedPoints.size(); i++){
        ...
        if (pcl::isFinite(msg->at(c,r)) && pcl::isFinite(normals->at(c,r))){
			...
			if (pcl::isFinite(msg->at(c-k, r)) && pcl::isFinite(msg->at(c+k, r)) 
				...
				if (c_concavity > 0 && r_concavity > 0 && get3Ddistance(msg->at(c,r),origin_ptr->at(c,r)) >= max_dis - 0.016) {
                ...
                #if 0
				else if( abs(c_concavity) <= 0.004&& abs(r_concavity) <= 0.004 && abs( common_z - min_z) <= 0.02 && abs(msg->at(c,r).z - common_z) <= 0.004){
...
void PointCloudAnalyzer::callback(const PointCloud::ConstPtr& msg)
{
    ...
    */
    ...
    std::cout<<"end depthCallback"<<endl;
